//
//  CABasicAnimationPositionViewController.m
//  AnimationDemo
//
...
#import "CABasicAnimationPositionViewController.h"

@interface CABasicAnimationPositionViewController ()
...
@implementation CABasicAnimationPositionViewController
...
{
    self.title = @"CABasicAnimationPosition相关动画";
}
...
// position 基本 动画
- (void)p_basicAnimationOfPosition
{
...
// position.x 基本 动画
- (void)p_basicAnimationOfPositionX {
    ...
    ani.fillMode = kCAFillModeForwards;
    ani.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
    [self.integralView.layer addAnimation:ani forKey:@"PositionXAni"];
}

// position.y 基本 动画
- (void)p_basicAnimationOfPositionY {
    CABasicAnimation *ani = [CABasicAnimation animationWithKeyPath:@"position.y"];
    ani.toValue = [NSNumber numberWithFloat:self.integralView.center.y];
    ani.removedOnCompletion = NO;
    ani.duration = 1.0f;
    ani.fillMode = kCAFillModeForwards;
    ani.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
    [self.signInView.layer addAnimation:ani forKey:@"PositionYAni"];
}
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if((self.count) % 3 == 0)
    {
        [self p_basicAnimationOfPosition];
    }
    else if(self.count % 3 == 1)
    {
        [self p_basicAnimationOfPositionX];
    }
